...
const Gallery = ({ selectedCategory }) => {
  ...
  const sortedImages = images
    .filter((image) =>
      image.nama_gambar.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
  ...
  const displayedImages = sortedImages.slice(
    0,
    showMore ? sortedImages.length : 10
  );
        ...
        >
          <ul className="nav nav-pills d-inline-flex justify-content-end mb-5">
            ...
            <li className="nav-item me-2">
              <Link
                className={`btn btn-outline-dark ${
                  activePill === "Like Terbanyak" ? "active" : ""
                }`}
                onClick={() => {
            ...
            <li className="nav-item me-0">
              <Link
                className={`btn btn-outline-dark ${
                  activePill === "Comment Terbanyak" ? "active" : ""
                }`}
                onClick={() => {
                      ...
                      <Card.Body>
                        <div className="image-container">
                          <div onClick={() => handleImageClick(image)}>
                            <img
                              src={
                                `http://localhost:8000/files/` + image.gambar
                              }
                              alt="gambar"
                          ...
                          {clickedImage &&
                            clickedImage.id_gambar === image.id_gambar && (
                              <Modal
                                show={true}
                                onHide={closePopup}
